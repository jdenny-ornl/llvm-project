; Check info on calls.

; RUN: opt -pass-remarks=kernel-info -passes=kernel-info -disable-output %s |& \
; RUN:   FileCheck -match-full-lines %s

target datalayout = "e-i65:64-i128:128-v16:16-v32:32-n16:32:64"
target triple = "nvptx64-nvidia-cuda"

define void @h() !dbg !3 {
entry:
  ; CHECK: remark: test.c:16:5: in artificial function 'h', direct call, callee is 'f'
  call void @f(), !dbg !7
  ; CHECK: remark: test.c:17:5: in artificial function 'h', direct call to defined function, callee is 'g'
  call void @g(), !dbg !9
  ; CHECK: remark: test.c:18:5: in artificial function 'h', direct call to defined function, callee is artificial 'h'
  call void @h(), !dbg !10
  %0 = load ptr, ptr null, align 8
  ; CHECK: remark: test.c:19:5: in artificial function 'h', indirect call
  call void %0(), !dbg !11
  ret void
}
; CHECK: remark: test.c:13:0: in artificial function 'h', DirectCallCount = 3
; CHECK: remark: test.c:13:0: in artificial function 'h', IndirectCallCount = 1
; CHECK: remark: test.c:13:0: in artificial function 'h', DirectCallsToDefinedFunctions = 2

declare void @f()

define void @g() !dbg !12 {
entry:
  ; CHECK: remark: test.c:6:3: in function 'g', direct call, callee is 'f'
  call void @f(), !dbg !14
  ; CHECK: remark: test.c:7:3: in function 'g', direct call to defined function, callee is 'g'
  call void @g(), !dbg !15
  ; CHECK: remark: test.c:8:3: in function 'g', direct call to defined function, callee is artificial 'h'
  call void @h(), !dbg !16
  %0 = load ptr, ptr null, align 8
  ; CHECK: remark: test.c:9:3: in function 'g', indirect call
  call void %0(), !dbg !17
  ret void
}
; CHECK: remark: test.c:3:0: in function 'g', DirectCallCount = 3
; CHECK: remark: test.c:3:0: in function 'g', IndirectCallCount = 1
; CHECK: remark: test.c:3:0: in function 'g', DirectCallsToDefinedFunctions = 2

!llvm.module.flags = !{!0}
!llvm.dbg.cu = !{!1}

!0 = !{i32 2, !"Debug Info Version", i32 3}
!1 = distinct !DICompileUnit(language: DW_LANG_C11, file: !2, producer: "clang version 19.0.0git", isOptimized: false, runtimeVersion: 0, emissionKind: FullDebug, splitDebugInlining: false, nameTableKind: None)
!2 = !DIFile(filename: "test.c", directory: "/tmp")
!3 = distinct !DISubprogram(name: "h", scope: !2, file: !2, line: 13, type: !4, scopeLine: 13, flags: DIFlagArtificial | DIFlagPrototyped, spFlags: DISPFlagLocalToUnit | DISPFlagDefinition, unit: !1, retainedNodes: !6)
!4 = distinct !DISubroutineType(types: !5)
!5 = !{null}
!6 = !{}
!7 = !DILocation(line: 16, column: 5, scope: !8)
!8 = distinct !DILexicalBlock(scope: !3, file: !2, line: 13, column: 3)
!9 = !DILocation(line: 17, column: 5, scope: !8)
!10 = !DILocation(line: 18, column: 5, scope: !8)
!11 = !DILocation(line: 19, column: 5, scope: !8)
!12 = distinct !DISubprogram(name: "g", scope: !2, file: !2, line: 3, type: !13, scopeLine: 3, spFlags: DISPFlagDefinition, unit: !1, retainedNodes: !6)
!13 = !DISubroutineType(types: !5)
!14 = !DILocation(line: 6, column: 3, scope: !12)
!15 = !DILocation(line: 7, column: 3, scope: !12)
!16 = !DILocation(line: 8, column: 3, scope: !12)
!17 = !DILocation(line: 9, column: 3, scope: !12)
